def knapsack01_iter(wts, vals, wt_limit):
    ...
    for i in range(1, len(wts) + 1):
        for j in range(1, wt_limit + 1):
            if j >= wts[i - 1]:
                ...
                #                    dp_arr[i - 1][j])
                new_val = vals[i - 1] + dp_arr[i - 1][j - wts[i - 1]]
                if new_val > dp_arr[i - 1][j]:
                    dp_arr[i][j] = new_val
                    wt_arr[i][j] = [wts[i - 1]] + wt_arr[i - 1][j - wts[i - 1]]
                else:
...
if __name__ == '__main__':
    ...
    # vals = [50, 110, 160]
    # wlimit = 5
    # print(knapsack01_iter(wts, vals, wlimit))
